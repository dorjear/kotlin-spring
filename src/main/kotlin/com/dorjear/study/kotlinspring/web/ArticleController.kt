package com.dorjear.study.kotlinspring.web

import com.dorjear.study.kotlinspring.model.Article
import com.dorjear.study.kotlinspring.model.ArticleRepository
...
class ArticleController(private val articleRepository: ArticleRepository) {
    ...
    fun getArticleById(@PathVariable(value = "id") articleId: Long): ResponseEntity<Article> {
        return articleRepository.findById(articleId).map { article ->ResponseEntity.ok(article)}.orElse(ResponseEntity.notFound().build())
    }
    ...
    fun updateArticleById(@PathVariable(value = "id") articleId: Long,@Valid @RequestBody newArticle: Article): ResponseEntity<Article> {
        return articleRepository.findById(articleId).map {
            existingArticle -> val updatedArticle: Article = existingArticle.copy(title = newArticle.title, content = newArticle.content)
